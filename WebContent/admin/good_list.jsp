...
<div class="container">
	...
	<ul role="tablist" class="nav nav-tabs">
        ...
        <li <c:if test='${status==1}'>class="active"</c:if> role="presentation"><a href="goodList?status=1">爆款推荐</a></li>
    </ul>
    
    <c:if test="${status == 1}"><br><p>首页默认只显示前[ 4 ]条记录</p></c:if>
	...
	<c:forEach var="good" items="${goodList}">
         <tr>
			...
			<td>
				<p>
					<c:if test="${good.show}"><a class="btn btn-info topDelete" href="javascript:;" type="1" goodid="${good.id}" text="加入爆款">移出爆款</a></c:if>
					<c:if test="${!good.show}"><a class="btn btn-primary topSave" href="javascript:;" type="1" goodid="${good.id}" text="移出爆款">加入爆款</a></c:if>
				</p>
