...
function shopPoint_add(id)
{
		...
		$.ajax({
			...
			type: "get",
			dataType: "json",
			data:{ id:id, act:"shopcartPoint_add", rn:random1(0,100000),totmoney:$('#totmoney').val(),inyunfei:$('#inyunfei').val(),intotal:$('#intotal').val()},
			success:function(data){
					
				if(data.con==0){
					$('#total').html(data.endtotal1);
					$('#intotal').val(data.codetotal);//
					$('#pointnum').val(data.password);//优惠券编号
					$('#youhuiquan1').html(data.conmoneym);
					$('#yh').val(data.conmoneym);
